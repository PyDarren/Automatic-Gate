...
if __name__ == '__main__':
    ...
    for info in file_names:
        ...
        df.columns = ['ID', 'Age', 'Immune Impairment Score']
        print(df)
        df.to_excel(raw_path + '/final_score/%s_immune_score.xlsx' % info[:-4],  index=False)
        all_score = all_score.append(df)

    all_score.to_excel(raw_path + '/final_score/allImmuneScore.xlsx', index=False)
